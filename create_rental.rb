class CreateRental
  ...
  def create_rental
    ...
    @people.each_with_index { |s, i| print "(#{i}) [#{s["Person"]}] Name: #{s["Name"]}, ID: #{s["ID"]}, Age: #{s["Age"]}\n" }
    person = gets.chomp.to_i
    new_rental = Rental.new(date, @books[book], @people[person])
    rentalObj = {"Title":new_rental.book["Title"], "Author":new_rental.book["Author"], "Date":new_rental.date, "ID":new_rental.person["ID"]}
    @rentals << rentalObj
